...
void run(nvbench::state& state) {
  ...
  cudf::io::parquet_reader_options read_opts = cudf::io::parquet_reader_options::builder(source_sink.make_source_info());
  ...
  state.exec(
    ...
    [&](nvbench::launch& launch, auto& timer) {
      ...
      for (int i = 0; i < streamCount; ++i) {
        threads.emplace_back([&] { cudf::io::read_parquet(read_opts); });
      }
      for (auto& t : threads) {
        t.join();
      }
